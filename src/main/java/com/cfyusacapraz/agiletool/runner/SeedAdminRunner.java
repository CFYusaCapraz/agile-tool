...
public class SeedAdminRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken("system", null, List.of(new SimpleGrantedAuthority(Roles.ROLE_ADMIN.name())));
        SecurityContextHolder.getContext().setAuthentication(authenticationToken);

        String email = seedAdminProperties.getEmail().trim().toLowerCase();

        try {
            UserDto existing = userService.getByEmail(email).join();
            if (existing != null) {
        ...
        userCreateRequest.setRole(Roles.ROLE_ADMIN);
    ...
    private boolean isNotFound(Throwable t) {
        ...
        while (cur != null) {
            ...
            if (className.contains("notfound") || className.contains("nosuch") || msg.contains("not found") || msg.contains("no such")) {
                return true;
            }
            cur = cur.getCause();
        }
